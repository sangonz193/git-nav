    ...
    <main className="relative flex min-h-svh items-center justify-center bg-background p-8">
      ...
      <section className="w-full max-w-lg space-y-7">
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Choose a recent repository or browse for another folder.
          </p>
        </div>
                  ...
                  <div key={project.id}>
                    <div className="flex items-stretch">
                      ...
                      <button
                        ...
                        onClick={() =>
                          openRepository(project.path).catch(
                            (message: unknown) => setError(String(message)),
                          )
                        }
                        type="button"
                      >
      ...
      <FolderPicker
        ...
        onChoose={(path) => {
          ...
          openRepository(path).catch((message: unknown) =>
            setError(String(message)),
          )
        }}
        open={isPickerOpen}
