...
export function LabelText({ label }: { label: Label }) {
  ...
  return (
    <span className="flex min-w-0">
      {label.before && (
        <span className="shrink-0 whitespace-pre">{label.before}</span>
      )}
      <RefName kind={label.kind} name={label.name} />
      {label.after && (
        <span className="shrink-0 whitespace-pre">{label.after}</span>
      )}
    </span>
  )
}
...
export function OperationMenuItems({
  components,
  ...
  const entries = applicableOperations(repository, source, target).filter(
    ({ operation }) => groups.includes(operation.group),
  )
  if (entries.length === 0) {
    return null
  }
  return (
    <>
      ...
      {entries.map(({ operation, request, unavailable }, index) => {
        ...
        const opensGroup =
          entries[index - 1]?.operation.group !== operation.group
        return (
...
export function OperationDialog({
  ...
  const { fields, values } = useMemo(
    () => resolveFields(operation, request, entered),
    [operation, request, entered],
  )
  const firstTextField = fields.find((field) => field.kind === "text")?.key
  const needs = useMemo(
    () => operation.needs?.(request, values) ?? {},
  ...
  const plan =
    blocks.length === 0 ? operation.plan(request, values, state) : null
  const { isPending, mutate } = useMutation({
    mutationFn: (plan: Plan) =>
      invoke<OperationResult>(plan.command, { repoPath, ...plan.args }),
    onSuccess: (result) => {
  ...
  useEffect(() => {
    ...
    let disposed = false
    invoke<BranchOperationState>("branch_operation_state", {
      repoPath,
      branch: needs.branch,
    })
      .then(
        (branch) =>
          !disposed && setState((current) => ({ ...current, branch })),
      )
      .catch(() => undefined)
    return () => {
      disposed = true
  ...
  useEffect(() => {
    ...
    invoke<string>("merge_base", { repoPath, left, right })
      .then(
        (mergeBase) =>
          !disposed && setState((current) => ({ ...current, mergeBase })),
      )
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      predictConflicts(repoPath, prediction)
        .then(
          (prediction) =>
            !disposed && setState((current) => ({ ...current, prediction })),
        )
        .catch(() => undefined)
    }, PREDICTION_DEBOUNCE)
    return () => {
  ...
  const waiting =
    (Boolean(needs.prediction) && state.prediction === null) ||
    (Boolean(needs.mergeBase) && state.mergeBase === null)
  ...
  return (
    <AlertDialog onOpenChange={(open) => !open && onClose()} open>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>
            {operationTitle(operation, request)}
          </AlertDialogTitle>
          <AlertDialogDescription>
            {operation.description(request, values)}
          </AlertDialogDescription>
        </AlertDialogHeader>
        {fields.length > 0 && (
        ...
        {plan && (
          <code className="overflow-x-auto rounded-lg border p-3 font-mono text-xs whitespace-pre">
            {plan.argv.join(" ")}
          </code>
        )}
        <AlertDialogFooter>
          ...
          >
            ...
            : waiting ?
              "Checking…"
            : operation.action(request, values)}
          </Button>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
