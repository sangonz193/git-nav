...
export const PENDING_OPERATION_LABELS = {
  bisect: "bisecting",
...
export function graphCanvasHeight(
  viewportHeight: number,
  headerHeight = GRAPH_HEADER_HEIGHT,
) {
...
export function graphCanvasTop(
  scrollTop: number,
  viewportHeight: number,
  ...
  return Math.min(
    scrollTop - GRAPH_CANVAS_OVERSCAN,
    headerHeight +
      contentHeight -
      graphCanvasHeight(viewportHeight, headerHeight),
  )
}
...
export function parentEdgeColor(
  commit: Commit,
  parentIndex: number,
  endLane: number,
) {
...
export function unpushedHashes(
  commits: Commit[],
  ...
  for (const commit of commits) {
    if (
      pushed.has(commit.hash) ||
      commit.refs.some((ref) => isRemoteRef(ref, remotes))
    ) {
...
export function startsLane(commits: Commit[], index: number, lane: number) {
  ...
  return (
    commit.parentLanes.includes(lane) &&
    (lane === commit.lane ||
      commit.incomingLanes?.includes(lane) ||
      !commits[index - 1]?.activeLanes[lane])
  )
}
...
export function unpushedLanes(commits: Commit[], unpushed: Set<string>) {
  ...
  return commits.map((commit, index) => {
    ...
    for (
      let lane = 0;
      lane < commit.activeLanes.length && lane < 31;
      lane += 1
    ) {
...
export function commitFromTuple([
  hash,
  parents,
  author,
  date,
...
export function commitSelection(
  ...
  return {
    ...
    base:
      baseHash ?
        (commits.find((commit) => commit.hash === baseHash) ?? null)
      : null,
  }
}

export function persistedSelectionRange(
  commits: Commit[],
  hashes: string[],
  isLoading: boolean,
) {
  ...
  const selected = hashes
    .map((hash) => commits.find((commit) => commit.hash === hash))
...
export function persistedSelectionHashes(
  range: { anchorHash: string; focusHash: string } | null,
) {
  if (!range) {
    return []
  }
...
export function persistedSelectionRestore(
  commits: Commit[],
  hashes: string[],
  isLoading: boolean,
) {
  ...
  return range === undefined ? undefined : (
      { range, selectedCommitHashes: persistedSelectionHashes(range) }
    )
}

export function persistedGraphPanelParams(
  name: string,
  path: string,
...
export function branchesContaining(
  ...
  for (let index = tipIndex; index >= 0; index -= 1) {
    ...
    if (
      index !== tipIndex &&
      commit.parents.some((parent) => reachesTip.has(parent))
    ) {
...
export function relativeDate(value: string) {
  ...
  return (relativeTimeFormatter ??= new Intl.RelativeTimeFormat(undefined, {
    numeric: "auto",
...
export function remoteBranchName(ref: DisplayRef) {
  return ref.remote && ref.label.startsWith(`${ref.remote}/`) ?
      ref.label.slice(ref.remote.length + 1)
    : null
}
...
export function detachedWorktrees(
  refs: DisplayRef[],
  worktrees: RowWorktree[],
) {
  const attached = new Set(
    refs.flatMap((ref) => ref.worktrees.map((worktree) => worktree.path)),
  )
...
function chipPriority(chip: RowChip) {
  ...
  return chip.kind === "worktree" ?
      CHIP_PRIORITY.worktree
    : refPriority(chip.ref)
}
...
export function textChipWidth(label: string, maxWidth: number) {
  return Math.min(
    maxWidth,
    CHIP_FIXED_WIDTH + label.length * CHIP_CHARACTER_WIDTH,
  )
}
...
function worktreeMarkerWidth(worktrees: RowWorktree[]) {
  return worktrees.reduce((total, worktree) => {
    ...
    return (
      ...
      CHIP_ICON_WIDTH +
      (changes > 0 ?
        CHIP_ICON_WIDTH + String(changes).length * CHIP_CHARACTER_WIDTH
      : 0)
    )
  }, 0)
}
...
export function chipWidth(chip: RowChip, maxWidth: number) {
  ...
  if (chip.kind === "worktree") {
    return Math.min(
      maxWidth,
      CHIP_FIXED_WIDTH +
        worktreeMarkerWidth([chip.worktree]) +
        chip.worktree.name.length * CHIP_CHARACTER_WIDTH,
    )
  }
  ...
  return Math.min(
    ...
    CHIP_FIXED_WIDTH +
      ...
      badge +
      sync +
      worktreeMarkerWidth(chip.ref.worktrees) +
      chip.ref.label.length * CHIP_CHARACTER_WIDTH,
  )
}
...
function chipsWidth(chips: RowChip[], maxWidth: number) {
  return chips.reduce(
    (total, chip, index) =>
      total + chipWidth(chip, maxWidth) + (index > 0 ? CHIP_GAP : 0),
    0,
  )
}
...
export function worktreeDescription(worktree: RowWorktree) {
  const changes = [
    worktree.changedFiles && `${worktree.changedFiles} changed`,
    worktree.untrackedFiles && `${worktree.untrackedFiles} untracked`,
  ]
    .filter(Boolean)
    .join(", ")
  return [
    worktree.isCurrent ? `${worktree.name} is this window's worktree`
    : worktree.isOpen ? `${worktree.name} is open in another window`
    : `Checked out at ${worktree.name}`,
    worktree.branch ? null : "Not on a branch",
    worktree.pendingOperation &&
      `Currently ${PENDING_OPERATION_LABELS[worktree.pendingOperation]}`,
    changes || null,
  ]
    .filter(Boolean)
    .join("\n")
...
export function splitRefLabel(label: string) {
  return label.length <= REF_TAIL_LENGTH ?
      { start: label, end: "" }
    : {
        start: label.slice(0, -REF_TAIL_LENGTH),
        end: label.slice(-REF_TAIL_LENGTH),
      }
}
...
export function syncDescription({ sync }: DisplayRef) {
  ...
  const counts = [
    sync.ahead && `${sync.ahead} ahead`,
    sync.behind && `${sync.behind} behind`,
  ].filter(Boolean)
  return counts.length > 0 ?
      `${sync.upstream}: ${counts.join(", ")}`
...
export function displayRefs(
  ...
  const checkedOut = refs
    .find((ref) => ref.startsWith("HEAD -> "))
    ?.slice("HEAD -> ".length)
  const branchRefs = refs.filter(
    (ref) => !ref.startsWith("HEAD -> ") && !ref.startsWith("tag: "),
  )
  const localBranches = new Set(
    branchRefs.filter((ref) => !remoteOf(ref, remotes)),
  )
