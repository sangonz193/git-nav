...
describe("initialDiffLayout", () => {
  test("uses explicit preferences without treating responsive state as one", () => {
    ...
    ).toEqual({ fileTreeOpen: false, mode: "split", wrap: true })
    expect(
      initialDiffLayout(700, { fileTreeOpen: false, mode: "unified" }),
    ).toEqual({ fileTreeOpen: false, mode: "unified", wrap: false })
    expect(
      initialDiffLayout(700, { fileTreeOpen: true, mode: "unified" }),
    ).toEqual({ fileTreeOpen: true, mode: "unified", wrap: false })
    expect(initialDiffLayout(1000, { wrap: true })).toEqual({
      fileTreeOpen: true,
      mode: "split",
      wrap: true,
    })
    expect(initialDiffLayout(500, { wrap: false })).toEqual({
      fileTreeOpen: false,
      mode: "unified",
      wrap: false,
    })
  ...
  test("derives every unpreferred layout value from the current width", () => {
    expect(initialDiffLayout(NARROW_DIFF_PANEL_WIDTH - 1, {})).toEqual({
      fileTreeOpen: false,
      mode: "unified",
      wrap: true,
    })
    expect(initialDiffLayout(NARROW_DIFF_PANEL_WIDTH, {})).toEqual({
      fileTreeOpen: true,
      mode: "unified",
      wrap: false,
    })
    expect(initialDiffLayout(1000, {})).toEqual({
      fileTreeOpen: true,
      mode: "split",
      wrap: false,
    })
...
describe("toggledDiffFileTree", () => {
  test("does not turn a transient narrow drawer state into a preference", () => {
    ...
    expect(toggledDiffFileTree(true, true, preferences)).toEqual({
      fileTreeOpen: false,
      preferences,
    })
    expect(toggledDiffFileTree(false, true, {})).toEqual({
      fileTreeOpen: true,
      preferences: {},
    })
  })
...
describe("isViewedFile", () => {
  ...
  test("holds a mark only while the file is still the patch it was read at", () => {
    ...
    expect(
      isViewedFile(file({ newOid: "c".repeat(40) }), "feature", viewed),
    ).toBe(false)
    expect(
      isViewedFile(file({ newPath: "src/other.ts" }), "feature", viewed),
    ).toBe(false)
  })
  ...
  test("drops a mark when the base moves under a head that has not", () => {
    const viewed = new Map([["src/index.ts", fileIdentity(file(), "feature")]])
    expect(
      isViewedFile(file({ oldOid: "c".repeat(40) }), "feature", viewed),
    ).toBe(false)
  })
  ...
  test("reads a deleted file at the blob it was deleted from", () => {
    ...
    expect(
      isViewedFile(
        deleted,
        "feature",
        new Map([["src/index.ts", fileIdentity(deleted, "feature")]]),
      ),
    ).toBe(true)
  })
  ...
  test("keeps a working tree mark only in memory", () => {
    ...
    expect(
      isViewedFile(
        file({ newOid: null }),
        WORKTREE_REF,
        new Map([["src/index.ts", "a".repeat(40)]]),
      ),
    ).toBe(false)
  })
  ...
  test("never carries a mark for a file with no blob behind it", () => {
    expect(
      isViewedFile(file({ oldOid: null, newOid: null }), "feature", new Map()),
    ).toBe(false)
  })
})

describe("isFoldedFile", () => {
  const file = (overrides: Partial<ChangedFile> = {}): ChangedFile => ({
    status: "modified",
    oldPath: "src/index.ts",
    newPath: "src/index.ts",
  ...
  test("folds a file once it has been read", () => {
    ...
    expect(isFoldedFile(file(), "feature", new Map(), new Map(), "key")).toBe(
      false,
    )
  })
  ...
  test("lets a fold set by hand answer the mark either way", () => {
    const read = new Map([["src/index.ts", fileIdentity(file(), "feature")]])
    expect(
      isFoldedFile(file(), "feature", read, new Map([["key", false]]), "key"),
    ).toBe(false)
    expect(
      isFoldedFile(
        file(),
...
describe("persistedDiffPanelParams", () => {
  ...
  test("keeps labels and only explicit user preferences", () => {
    expect(
      persistedDiffPanelParams(repository, refs, "src/index.ts", {
        fileTreeOpen: true,
        mode: "split",
      }),
    ).toEqual({
