...
// it can be read back.
type Checks<Fields> = {
  [Key in keyof Required<Fields>]: Check<Required<Fields>[Key]>
}
type RequiredKeys<Fields> = {
  [Key in keyof Fields]-?: object extends Pick<Fields, Key> ? never : Key
}[keyof Fields]
...
const DIFF_PREFERENCE_MANDATORY =
  {} satisfies Mandatory<DiffPanelUserPreferences>
const GRAPH_PREFERENCE_MANDATORY =
  {} satisfies Mandatory<GraphPanelUserPreferences>
const DIFF_PARAM_MANDATORY = {
  baseRef: true,
  headRef: true,
  name: true,
  path: true,
} satisfies Mandatory<DiffPanelParams>
const GRAPH_PARAM_MANDATORY = {
  name: true,
  path: true,
...
const DIFF_PARAM_CHECKS = {
  ...
  userPreferences: (value): value is DiffPanelUserPreferences =>
    validFields(value, DIFF_PREFERENCE_CHECKS, DIFF_PREFERENCE_MANDATORY),
} satisfies Checks<DiffPanelParams>

const GRAPH_PARAM_CHECKS = {
  name: isString,
  path: isString,
  selectedCommitHashes: (value): value is string[] =>
    Array.isArray(value) && value.every(isString),
  userPreferences: (value): value is GraphPanelUserPreferences =>
...
function validFields(
  ...
  mandatory: Record<string, true>,
) {
  if (!isObject(value)) {
    return false
  }
  return (
    Object.keys(mandatory).every((key) => value[key] !== undefined) &&
    Object.entries(value).every(
      ([key, field]) => Object.hasOwn(checks, key) && checks[key](field),
    )
  )
}
...
function validGraphParams(params: PanelParams, path: string) {
  return (
    params.path === path &&
    validFields(params, GRAPH_PARAM_CHECKS, GRAPH_PARAM_MANDATORY)
  )
}
...
export function usableRepositoryLayout(
  ...
  if (
    !isObject(value) ||
    value.version !== REPOSITORY_LAYOUT_VERSION ||
  ...
  try {
    await Promise.race([
      ...
      new Promise<void>((resolve) => {
        timeout = globalThis.setTimeout(resolve, saveTimeout)
      }),
    ])
  } finally {
...
export function repositoryLayoutSaveScheduler(
...
export function listenForRepositoryLayoutPageHide(
...
export async function unresolvablePanelIds(
  layout: SerializedDockview,
  resolveRevision: (path: string, revision: string) => Promise<unknown>,
) {
  const resolutions = await Promise.all(
    Object.entries(layout.panels).map(async ([id, panel]) => {
      const panelRevision = panelRevisions(panel as SerializedPanel)
      if (!panelRevision) {
        return null
      }
      const resolved = await Promise.all(
        panelRevision.revisions.map((revision) =>
          resolveRevision(panelRevision.path, revision)
            .then(() => true)
            .catch(() => false),
        ),
      )
...
export function restoreRepositoryLayout<Panel extends RestorablePanel>(
  container: RestorableContainer<Panel>,
  layout: SerializedDockview,
