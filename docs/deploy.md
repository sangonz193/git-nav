...
The workflow builds, tests, packages, and publishes `@git-nav/darwin-arm64`, `@git-nav/linux-x64`, `@git-nav/linux-arm64`, `@git-nav/win32-x64`, and `@git-nav/win32-arm64` before publishing `git-nav`. Linux platform packages contain a Tauri AppImage named `git-nav.AppImage`; Windows platform packages contain `git-nav.exe`. The `git-nav` launcher runs the matching native executable directly.
